...
esp_err_t ApiConfig::get_config_handler(httpd_req_t* req) {
    ...
    if (err != ESP_OK) {
        ...
        int written = ApiResponse::build_error_response(
            response, MAX_RESPONSE_SIZE, ApiResponse::StatusCode::INTERNAL_SERVER_ERROR,
            "Failed to open NVS");
...
esp_err_t ApiConfig::put_config_handler(httpd_req_t* req) {
    ...
    while (cur_len < sizeof(buffer) - 1) {
        cur_len =
            espHttpServer_.httpd_req_recv(req, buffer + total_len, sizeof(buffer) - 1 - total_len);
        if (cur_len <= 0) {
    ...
    if (has_device_name && !ApiValidator::is_valid_device_name(device_name)) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        ApiResponse::build_error_response(
            response, MAX_RESPONSE_SIZE, ApiResponse::StatusCode::BAD_REQUEST,
