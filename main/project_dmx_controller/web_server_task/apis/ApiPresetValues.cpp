...
esp_err_t ApiPresetValues::get_preset_values_handler(httpd_req_t* req) {
    ...
    uint32_t preset_num =
        UriParamExtractor::extractFirstParamAsUint32("/api/v1/preset_values/*", req->uri);

    auto preset = presetManager_.get_preset(preset_num);
    if (!preset) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        espHttpServer_.httpd_resp_send(req, "{\"error\":\"preset not found\"}",
...
esp_err_t ApiPresetValues::set_preset_value_handler(httpd_req_t* req) {
    ...
    if (params.size() < 2) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        espHttpServer_.httpd_resp_send(req, "{\"error\":\"missing parameters\"}",
                                       HTTPD_RESP_USE_STRLEN);
    ...
    if (channel_idx >= 512) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        espHttpServer_.httpd_resp_send(req, "{\"error\":\"channel index out of range\"}",
                                       HTTPD_RESP_USE_STRLEN);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (err != ESP_OK) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        espHttpServer_.httpd_resp_send(req, "{\"error\":\"failed to save preset\"}",
                                       HTTPD_RESP_USE_STRLEN);
        return err;
    }
    ...
    if (err != ESP_OK) {
        espHttpServer_.httpd_resp_set_type(req, "application/json");
        espHttpServer_.httpd_resp_send(req, "{\"error\":\"failed to commit\"}",
                                       HTTPD_RESP_USE_STRLEN);
        return err;
    }

    std::string resp =
        "{\"index\":" + std::to_string(channel_idx) + ",\"value\":" + std::to_string(value) + "}";
    espHttpServer_.httpd_resp_set_type(req, "application/json");
