...
esp_err_t ApiConfiguration::get_configuration_handler(httpd_req_t *req) {
    ...
    snprintf(response, sizeof(response),
             "{\"wifi_ssid\":\"%s\",\"device_name\":\"%s\",\"circular navigation\":%s}", ssid,
             device_name, circular_nav ? "true" : "false");
...
esp_err_t ApiConfiguration::set_circular_navigation_handler(httpd_req_t *req) {
    ...
    char response[128];
    snprintf(response, sizeof(response), "{\"circular navigation\":%s}", value ? "true" : "false");
