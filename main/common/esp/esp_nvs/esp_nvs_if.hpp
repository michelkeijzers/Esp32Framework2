...
#ifdef ESP_PLATFORM
// Real ESP-IDF NVS types
#include <nvs.h>
#include <nvs_flash.h>

#else
// Stub types for non-ESP32 builds (PC, unit tests)
...
#define ESP_ERR_NVS_NOT_FOUND                                                                       \
    (ESP_ERR_NVS_BASE + 0x02) /*!< A requested entry couldn't be found or namespace doesn’t exist \
                                 yet and mode is NVS_READONLY */
#define ESP_ERR_NVS_TYPE_MISMATCH \
    (ESP_ERR_NVS_BASE +           \
     0x03) /*!< The type of set or get operation doesn't match the type of value stored in NVS */
...
#define ESP_ERR_NVS_NOT_ENOUGH_SPACE \
    (ESP_ERR_NVS_BASE +              \
     0x05) /*!< There is not enough space in the underlying storage to save the value */
...
#define ESP_ERR_NVS_REMOVE_FAILED                                                                  \
    (ESP_ERR_NVS_BASE +                                                                            \
     0x08) /*!< The value wasn’t updated because flash write operation has failed. The value was \
...
#define ESP_ERR_NVS_PAGE_FULL \
    (ESP_ERR_NVS_BASE + 0x0a) /*!< Internal error; never returned by nvs API functions */
#define ESP_ERR_NVS_INVALID_STATE                                                                \
    (ESP_ERR_NVS_BASE + 0x0b) /*!< NVS is in an inconsistent state due to a previous error. Call \
                                 nvs_flash_init and nvs_open again, then retry. */
#define ESP_ERR_NVS_INVALID_LENGTH \
    (ESP_ERR_NVS_BASE + 0x0c) /*!< String or blob length is not sufficient to store data */
#define ESP_ERR_NVS_NO_FREE_PAGES                                                               \
    (ESP_ERR_NVS_BASE +                                                                         \
     0x0d) /*!< NVS partition doesn't contain any empty pages. This may happen if NVS partition \
              was truncated. Erase the whole partition and call nvs_flash_init again. */
#define ESP_ERR_NVS_VALUE_TOO_LONG                                                              \
    (ESP_ERR_NVS_BASE + 0x0e) /*!< Value doesn't fit into the entry or string or blob length is \
                                 longer than supported by the implementation */
#define ESP_ERR_NVS_PART_NOT_FOUND \
    (ESP_ERR_NVS_BASE +            \
     0x0f) /*!< Partition with specified name is not found in the partition table */

#define ESP_ERR_NVS_NEW_VERSION_FOUND                                                      \
    (ESP_ERR_NVS_BASE + 0x10) /*!< NVS partition contains data in new format and cannot be \
                                 recognized by this version of code */
#define ESP_ERR_NVS_XTS_ENCR_FAILED \
    (ESP_ERR_NVS_BASE + 0x11) /*!< XTS encryption failed while writing NVS entry */
#define ESP_ERR_NVS_XTS_DECR_FAILED \
    (ESP_ERR_NVS_BASE + 0x12) /*!< XTS decryption failed while reading NVS entry */
...
#define ESP_ERR_NVS_ENCR_NOT_SUPPORTED \
    (ESP_ERR_NVS_BASE + 0x15) /*!< NVS encryption is not supported in this version */
...
#define ESP_ERR_NVS_WRONG_ENCRYPTION                                                        \
    (ESP_ERR_NVS_BASE +                                                                     \
     0x19) /*!< NVS partition is marked as encrypted with generic flash encryption. This is \
              forbidden since the NVS encryption works differently. */

#define ESP_ERR_NVS_CONTENT_DIFFERS                                                              \
    (ESP_ERR_NVS_BASE + 0x18) /*!< Internal error; never returned by nvs API functions.  NVS key \
                                 is different in comparison */

#define NVS_DEFAULT_PART_NAME \
    "nvs" /*!< Default partition name of the NVS partition in the partition table */

#define NVS_PART_NAME_MAX_SIZE \
    16 /*!< maximum length of partition name (excluding null terminator) */
#define NVS_KEY_NAME_MAX_SIZE                                          \
    16 /*!< Maximum length of NVS key name (including null terminator) \
        */
#define NVS_NS_NAME_MAX_SIZE \
    NVS_KEY_NAME_MAX_SIZE /*!< Maximum length of NVS namespace name (including null terminator) */
