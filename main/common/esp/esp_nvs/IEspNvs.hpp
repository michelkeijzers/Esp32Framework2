...
class IEspNvs {
   public:
    ...
    // Handle operations
    virtual esp_err_t nvs_open(const char *namespace_name, nvs_open_mode_t open_mode,
                               nvs_handle_t *out_handle) = 0;
    ...
    virtual esp_err_t nvs_get_str(nvs_handle_t handle, const char *key, char *out_value,
                                  size_t *length) = 0;
    virtual esp_err_t nvs_get_blob(nvs_handle_t handle, const char *key, void *out_value,
                                   size_t *length) = 0;
    ...
    virtual esp_err_t nvs_set_blob(nvs_handle_t handle, const char *key, const void *value,
                                   size_t length) = 0;
