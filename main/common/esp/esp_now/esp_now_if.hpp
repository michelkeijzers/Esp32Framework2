...
inline constexpr std::size_t kEspNowMacAddressLength = 6U;
...
inline constexpr int kEspNowMaxTotalPeerNum = 20;
inline constexpr int kEspNowMaxDataLen = 250;

#define ESP_NOW_MAX_TOTAL_PEER_NUM (kEspNowMaxTotalPeerNum)
#define ESP_NOW_MAX_DATA_LEN (kEspNowMaxDataLen)

typedef int wifi_second_chan_t;
typedef int esp_now_send_status_t;

typedef struct {
    uint8_t peer_addr[kEspNowMacAddressLength];
    uint8_t channel;
...
typedef void (*esp_now_recv_cb_t)(const esp_now_recv_info_t* esp_now_info, const uint8_t* data,
...
inline esp_err_t esp_now_del_peer(const uint8_t* peer_address) {
    (void)peer_address;
    return ESP_OK;
}
...
#ifdef ESP_PLATFORM
inline esp_err_t esp_now_send_payload(const uint8_t* peer_address,
                                      std::span<const uint8_t> payload) {
    return esp_now_send(peer_address, payload.data(), payload.size());
}
#else
inline esp_err_t esp_now_send_payload(const uint8_t* peer_address,
                                      std::span<const uint8_t> payload) {
