...
esp_err_t EspNow::sendMessage(std::span<const uint8_t> data)
{
    ...
    return esp_now_send(nullptr, data.data(), data.size());
}

esp_err_t EspNow::registerSendCallback(esp_now_send_cb_t cb)
{
    return esp_now_register_send_cb(cb);
}

esp_err_t EspNow::registerReceiveCallback(esp_now_recv_cb_t cb)
{
