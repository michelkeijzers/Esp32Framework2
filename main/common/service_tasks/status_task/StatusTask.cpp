...
void StatusTask::handleMessage(const StatusTaskMessages::MessageEnvelope &msg)
{
    ...
    case StatusTaskMessages::Type::SET_TASK_NAME:
    {
        ...
        auto it = std::find_if(taskData.begin(), taskData.end(), [&](const TaskData &t)
...
void StatusTask::handleHeartbeatTimer(TickType_t now)
{
    ...
    for (const auto &task : taskData)
    {
        if ((now - task.getLastHeartbeatTick()) >= timeoutTicks)
        {
            ...
            std::strncpy(timeoutMsg.data.heartbeatTimeout.task_name, task.getName(), StatusTaskMessages::TASK_NAME_MAX_LEN);
            timeoutMsg.data.heartbeatTimeout.task_name[StatusTaskMessages::TASK_NAME_MAX_LEN - 1] = '\0';
