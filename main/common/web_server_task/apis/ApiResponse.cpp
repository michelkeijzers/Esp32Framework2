...
int ApiResponse::build_success_response(char* buffer, size_t buffer_size, const char* data) {
    ...
    return snprintf(buffer, buffer_size, "{\"success\":true,\"data\":%s}", data);
}

int ApiResponse::build_error_response(char* buffer, size_t buffer_size, StatusCode status_code,
    ...
    return snprintf(buffer, buffer_size,
                    "{\"success\":false,\"error\":{\"code\":%d,\"message\":\"%s\"}}", status,
                    error_message);
}

int ApiResponse::get_http_status(StatusCode code) { return static_cast<int>(code); }
