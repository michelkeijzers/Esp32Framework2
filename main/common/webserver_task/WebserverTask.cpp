...
WebserverTask::WebserverTask(IApiServer &apiServer)
    : RtosTask(WEBSERVER_TASK_NAME, WEBSERVER_TASK_STACK_SIZE, WEBSERVER_TASK_PRIORITY)
...
{
    // TODO: perform any pre-start initialisation (e.g. Wi-Fi ready check)
    return ESP_OK;
}
...
{
    apiServer_.start();
    // TODO: create FreeRTOS task if ongoing work is needed beyond
    //       the initial server start (e.g. keep-alive monitoring)
    return ESP_OK;
}
