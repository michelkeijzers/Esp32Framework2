...
bool RtosQueue::send(const void* item, uint32_t timeoutMs) {
    ...
    BaseType_t res = xQueueSend(m_queueHandle, item, pdMS_TO_TICKS(timeoutMs));
    return res == pdPASS;
}

bool RtosQueue::receive(void* item, uint32_t timeoutMs) {
    if (!m_queueHandle) return false;
    BaseType_t res = xQueueReceive(m_queueHandle, item, pdMS_TO_TICKS(timeoutMs));
    return res == pdPASS;
}
...
size_t RtosQueue::count() const {
    ...
    return static_cast<size_t>(uxQueueMessagesWaiting(m_queueHandle));
}
