...
class ApiServerTest : public ::testing::Test {
   protected:
    ...
    NodesStaticInfo nodesStaticInfo{
        std::vector<NodeStaticInfo>{NodeStaticInfo(1, "Node", "fw", "cfg", "aa:bb", "1.2.3.4")}};
};
...
TEST_F(ApiServerTest, StartMountsFileSystemStartsServerAndRegistersEndpoints) {
    ...
    EXPECT_NE(std::find(registeredUris.begin(), registeredUris.end(), "/api/v1/logging"),
              registeredUris.end());
    EXPECT_NE(std::find(registeredUris.begin(), registeredUris.end(), "/*"), registeredUris.end());
...
TEST_F(ApiServerTest, StartLogsErrorWhenHttpServerStartFails) {
    ...
    EXPECT_CALL(mockEspFactory.logger_,
                log_error(StrEq("ApiServer"), StrEq("Failed to start HTTP server")))
        .Times(1);
...
TEST_F(ApiServerTest, StartFormatsLittleFsAndRetriesMountWhenInitialMountFails) {
    EXPECT_CALL(mockCommonApiFactory.apiNodes_, set_nodes_static_info(_)).Times(1);
    ApiServer apiServer(mockEspFactory, mockCommonApiFactory, nodesStaticInfo);
    auto serverHandle = reinterpret_cast<httpd_handle_t>(0x77);
...
TEST_F(ApiServerTest, StaticFileHandlerReturns404WhenFileAndFallbackAreMissing) {
    ...
    EXPECT_CALL(mockEspFactory.httpServer_, httpd_resp_send_404(&request))
        .Times(1)
        .WillOnce(Return(ESP_OK));
...
TEST_F(ApiServerTest, StaticFileHandlerThunkDelegatesToInstanceHandler) {
    ...
    EXPECT_EQ(ESP_OK, ApiServer::static_file_handler_thunk(&request));
    EXPECT_EQ("via thunk", body);
